// generated from rosidl_generator_c/resource/idl.h.em
// with input from crazyflie_interfaces:msg/AttitudeSetpoint.idl
// generated code does not contain a copyright notice

#ifndef CRAZYFLIE_INTERFACES__MSG__ATTITUDE_SETPOINT_H_
#define CRAZYFLIE_INTERFACES__MSG__ATTITUDE_SETPOINT_H_

#include "crazyflie_interfaces/msg/detail/attitude_setpoint__struct.h"
#include "crazyflie_interfaces/msg/detail/attitude_setpoint__functions.h"
#include "crazyflie_interfaces/msg/detail/attitude_setpoint__type_support.h"

#endif  // CRAZYFLIE_INTERFACES__MSG__ATTITUDE_SETPOINT_H_
