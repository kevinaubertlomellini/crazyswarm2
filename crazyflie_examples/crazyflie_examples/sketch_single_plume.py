...
class Drone:
    ...
    def update_velocity(self, desired_velocity, cf,uav,aux):
        ...
        Kp = 0.1
        cf.velWorld([velocity_scalar * desired_velocity[0], velocity_scalar * desired_velocity[1], Kp*(altitud-aux[2])], 0.0)
...
def run_sketch(df1p, df2p, offset, lambda_value, plumes, threshold, iterations, base_file_name=None,
    ...
    configs = [
        {'drone_id': drone1_id, 'starting_position': [1.0, -0.95, 0.75]},
        {'drone_id': drone2_id, 'starting_position': [1.2, -0.95, 1.25]}
    ]
    ...
    try:
        ...
        for drone in drones:
            drone.move_to_start_pos(6.0)

        timeHelper.sleep(9.0)

        step_time_save = np.zeros((iterations, 1))

        for i in range(iterations):
            start_time = timeHelper.time()  # Record start time
            # print(i)
            for drone in drones:
                rclpy.spin_once(Vel_subscribers[drone.drone_id], timeout_sec=0.1)
                rclpy.spin_once(Pos_subscribers[drone.drone_id], timeout_sec=0.1)
            ...
            for drone in drones:
                if drone.drone_id == drone1_id:
                    df1v_des = df1.update_velocity(np.array(df1SetVelocity.pipe(ops.first()).run()) /65, cf0,0,aux1)
                    ...
                    df2v_des = df2.update_velocity(np.array(df2SetVelocity.pipe(ops.first()).run()) /65, cf1,1,aux2)
            ...
            end_time = timeHelper.time()  # Record end time
            elapsed_time = end_time - start_time  # Calculate elapsed time
            step_time_save[i, 0] = elapsed_time

    except KeyboardInterrupt:
        print("Keyboard interrupt, triggering landing sequence")

    print('step_time:',np.mean(step_time_save))
    time.sleep(0.1)
    ...
    plt.plot(step_time_save)

    plt.show()

    rclpy.shutdown()

def plot_plumes_temp(df1p, df2p, threshold, plumes):
    ...
    for i in range(1):
        ...
        plot_plumes(plt, threshold + (i * 10), lon=[-106.59725, -106.59714], lat=[35.19725, 35.19732],
                                       plumes=plumes)
...
def main(args=None) -> None:
    ...
    df1p = LatLon(35.197285, -106.597181)
    df2p = LatLon(35.197285, -106.597181)


    run_sketch(df1p, df2p, 0.2, 0.005, plumes, threshold, iterations=6000, base_file_name='single_plume')
    #plot_plumes_temp(df1p, df2p, threshold, plumes)
