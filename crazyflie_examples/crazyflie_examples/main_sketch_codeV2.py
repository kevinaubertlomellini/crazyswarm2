...
class PathPublisher(Node):
    ...
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(Path, 'CF_1', 10)
...
class PathPublisher2(Node):

    def __init__(self):
        super().__init__('path_publisher')
        self.publisher_ = self.create_publisher(Path, 'CF_2', 10)
        timer_period = 2.0  # segundos
        self.timer = self.create_timer(timer_period, self.publish_path)
        self.get_logger().info('Path publisher node started')
...
class Drone:
    ...
    def update_position(self, cf_position, crazyflie):
        ...
        if crazyflie == 1:
            ...
            longitude = self.start_lat_lon.longitude + ((cf_position[0]-1.1) / (
                    math.cos(self.start_lat_lon.latitude * 0.01745) * (EARTH_CIRCUMFERENCE / 360)))
            latitude = self.start_lat_lon.latitude + ((cf_position[1] + 1.2) / (EARTH_CIRCUMFERENCE / 360))
...
def run_sketch(df1p, df2p, offset, lambda_value, plumes, threshold, iterations, base_file_name=None,
               starting_direction=None, latest_gradient=None, zoom=None, rep=1):
    swarm = Crazyswarm()
    ...
    path_publisher2 = PathPublisher2()
    ...
    configs = [
        ...
        {'drone_id': drone2_id, 'starting_position': [1.1, -1.2, 1.0]}
    ]
    ...
    rclpy.spin_once(path_publisher2, timeout_sec=0.1)
    ...
    experiment_directory= plot_plumes(plt, threshold, lon=[-106.59725, -106.59714], lat=[35.19725, 35.19732], plumes=plumes, rep=rep+1)
    ...
    try:
        ...
        for drone in drones:
            ...
            if drone.drone_id == drone1_id:
                X1 = drone.position[4:, 0]
                Y1 = drone.position[4:, 1]
                rclpy.spin_once(path_publisher, timeout_sec=0.1)


            elif drone.drone_id == drone2_id:
                X2 = drone.position[4:, 0]
                Y2 = drone.position[4:, 1]
                rclpy.spin_once(path_publisher2, timeout_sec=0.1)
...
def main(rep=1) -> None:
    ...
    df1p = LatLon(35.197282, -106.5971784)
    df2p = LatLon(35.197282, -106.5971754)

    run_sketch(df1p, df2p, 0.3, 0.001, plumes, threshold, iterations=500, base_file_name='single_plume',rep=rep)
