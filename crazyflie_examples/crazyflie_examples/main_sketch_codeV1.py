...
class Drone:
    ...
    def update_position(self, cf_position, crazyflie):
        ...
        if crazyflie == 1:
            longitude = self.start_lat_lon.longitude + ((cf_position[0]-0.9) / (
...
def run_sketch(df1p, df2p, offset, lambda_value, plumes, threshold, iterations, base_file_name=None,
    ...
    configs = [
        {'drone_id': drone1_id, 'starting_position': [0.9, -1.0, 1.0]},
        {'drone_id': drone2_id, 'starting_position': [1.2, -1.0, 1.0]}
    ]
    ...
    experiment_directory= plot_plumes(plt, threshold, lon=[-106.59725, -106.59704], lat=[35.19720, 35.19732], plumes=plumes, rep=rep+1)
...
def main(rep=1) -> None:
    plumes = [
        Plume(LatLon(35.19727, -106.5972), -30, 4170, 1.3),
        Plume(LatLon(35.197271, -106.597163), -35, 2100, 1.2)
    ]

    threshold = 485

    df1p = LatLon(35.197218, -106.5971359)
    df2p = LatLon(35.197218, -106.5971329)
    #df2p = LatLon(35.19722, -106.5971599)

    run_sketch(df1p, df2p, 0.3, 0.001, plumes, threshold, iterations=1300, base_file_name='single_plume',rep=rep)
