...
def calculate_co2_xy(latitude, longitude, plumes):
    return plumes.calculate_co2(LatLon(latitude, longitude))
...
class Plume:
    ...
    def calculate_co2(self, position):
        value = self.calculate_co2_raw(position)
    ...
    def calculate_co2_raw(self, position):
        [y, x] = rotate_vector(difference_in_meters(position, self.source), self.wind_direction * math.pi / 180)
        if x < 0:

            # Simple gaussian plume model adapted from: https://epubs.siam.org/doi/pdf/10.1137/10080991X
            # See equation 3.10, page 358.
            h = 2 # m Height
            return (self.q / (2 * math.pi * self.k * -x)) * math.exp(- (self.u * ((pow(y, 2) + pow(h, 2))) / (4 * self.k * -x)))
        else:
            return 0
...
class Plumes:
    ...
    def calculate_co2(self, position):
        ...
        for plume in self.plumes:
            value += plume.calculate_co2_raw(position)
    ...
    def markers(self):
        markers = []
        for plume in self.plumes:
            markers.extend(plume.markers())
        return markers

class StraightLinePlume:

    def calculate_co2(self, position):
        latitude_contribution = 450.0 - 50000 * (35.19573 - position.latitude)
        longitude_contribution = 450.0 + 50000 * (-106.59757 - position.longitude)

        # Assuming you want to average the contributions at 45° angle
        return (latitude_contribution + longitude_contribution) / 2
...
class SketchAction:
    ...
    def __init__(self, id, local_setvelocity_publisher, announce_stream, distance_sqrt_lambda, lambda_value, partner, leader, threshold,
                 drone_stream_factory, dragonfly_sketch_subject, position_vector_publisher, starting_direction=None, latest_gradient=None):
    ...
    def calculate_turn(self, partner_position, self_position, position_vector):
        if position_vector.movement == TURN:
            ...
            # Compute Centripetal Force using v^2 / r
...
def plot_plumes(plt, threshold, plumes, lon=[-106.598, -106.595], lat=[35.195, 35.1976], scale=100.0):
    plot_plumes_ax(plt.gca(), threshold, plumes, lon, lat, scale)

def plot_plumes_ax(ax, threshold, plumes, lon=[-106.598, -106.595], lat=[35.195, 35.1976], scale=100.0):
...
def main():
    ...
    plot_plumes(plt.gca(), threshold, plumes)
    ...
    for plume in plumes:
        plt.plot(plume.source.longitude, plume.source.latitude, marker='*', c='r',markeredgewidth=1, markeredgecolor=(0, 0, 0, 1), markersize=12)
